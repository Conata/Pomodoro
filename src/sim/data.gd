...
# ── ステージ制（タスクバーヒーロー準拠）───────────────────────────────
# 1幕＝3階（バイオーム一巡）。表記は「幕-番号」（例 2-3 ＝ B6F）。
# 難易度は敵と報酬に同じ倍率が乗る。前難易度で第1幕（3階）突破で次が開く。
const ACT_LEN := 3
const DIFFICULTIES := [
	{"name": "ノーマル",   "mult": 1.0,  "color": Color(0.45, 0.90, 0.50)},
	{"name": "ナイトメア", "mult": 2.5,  "color": Color(0.66, 0.40, 1.00)},
	{"name": "ヘル",       "mult": 6.0,  "color": Color(1.00, 0.55, 0.30)},
	{"name": "トーメント", "mult": 15.0, "color": Color(1.00, 0.30, 0.40)},
]


## 0始まりの階番号 → 「幕-番号」表記。
static func stage_label(fl: int) -> String:
	return "%d-%d" % [int(fl / float(ACT_LEN)) + 1, fl % ACT_LEN + 1]
