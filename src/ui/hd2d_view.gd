...
func _update_camera(instant: bool) -> void:
	...
	if _shake > 0.004:
		...
		_shake = maxf(0.0, _shake - get_process_delta_time() * 1.4)
	_cam.look_at(look + Vector3(0, 1.0, 0), Vector3.UP)
	# 戦闘中：敵が位相ずらしで手前へ踏み込む（生きた対峙感）
	if _combat:
		for i in _enemy_nodes.size():
			if _enemy_nodes[i].visible and i < _enemy_base.size():
				var lunge := maxf(0.0, sin(_pulse * 4.2 + i * 1.7)) * 0.4
				(_enemy_nodes[i] as Node3D).position = (_enemy_base[i] as Vector3) + Vector3(0, 0, lunge)
