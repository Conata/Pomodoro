...
func _apply_phase() -> void:
	...
	for ov in [management_overlay, formation_overlay, market_overlay, ship_overlay]:
...
func _build_management_overlay() -> void:
	...
	hcol.add_child(_label("箱の開封・会話・編成・闇市・交易船", TYPE_SMALL, COL_DIM))
...
func _build_morning(parent: Control) -> void:
	...
	# 箱バッジ付き経営アイコン
	var mgmt_wrap := VBoxContainer.new()
	mgmt_wrap.add_theme_constant_override("separation", 2)
	var mgmt_icon_btn := _home_icon_btn("経営", "📦", _open_management)
	home_box_badge = _label("", 10, Color(1.0, 0.7, 0.2))
	home_box_badge.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	mgmt_wrap.add_child(mgmt_icon_btn)
	mgmt_wrap.add_child(home_box_badge)
	right_nav.add_child(mgmt_wrap)
	...
	char_row.set_offset(SIDE_LEFT, 12); char_row.set_offset(SIDE_TOP, -360)
	...
	for cid in KuroData.GIRL_ORDER:
		...
		fc_wrap.custom_minimum_size = Vector2(80, 140)
	...
	bottom_area.set_offset(SIDE_LEFT, 12); bottom_area.set_offset(SIDE_TOP, -210)
...
func _refresh_morning() -> void:
	...
	_clear(girls_box)
	if not KuroData.GIRLS.has(_formation_sel):
		_formation_sel = KuroData.GIRL_ORDER[0]
	...
	if mgmt_banner_sub != null and is_instance_valid(mgmt_banner_sub):
		var box_n: int = s["boxes"].size()
		mgmt_banner_sub.text = "箱 %d個" % box_n if box_n > 0 else "編成・闇市・交易船・好感度"
	# 経営サブ画面が開いていれば中身も更新
...
## 店の運用UI（箱リスト・闇市・交易船・好感度）を box に充填。
func _fill_ops(box: VBoxContainer) -> void:
	var s := sim.state
	var boxes: Array = s["boxes"]
	var box_count: int = boxes.size()

	# ── ギフトボックス リスト ────────────────────────────
	box.add_child(_section("ギフトボックス"))
	# ヘッダー：合計数＋一括開封ボタン
	...
	for gid in KuroData.GIRL_ORDER:
		...
		aff_row.add_child(_label(String(gg["name"]), TYPE_SMALL, gg["color"]))
		var h: int = clampi(int(round(sim.aff(gid) / 20.0)), 0, 5)
		aff_row.add_child(_label("♥".repeat(h) + "♡".repeat(5 - h), TYPE_SMALL, Color("e88fb0")))
		var sp := Control.new()
		sp.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		aff_row.add_child(sp)
		aff_row.add_child(_label("%d pt" % sim.aff(gid), TYPE_SMALL, COL_DIM))
		box.add_child(aff_row)
...
## モバイルRPGスタイル編成ビュー。
## 上部タブでキャラ選択、中央大型立ち絵、左右スキルスロット、下部ステータス＋店番トグル。
func _girl_hero_panel(id: String) -> VBoxContainer:
	...
	for gid in KuroData.GIRL_ORDER:
		...
		icon.mouse_filter = Control.MOUSE_FILTER_IGNORE
		tab_col.add_child(icon)
		var name_lbl := _label(String(gg["name"]), TYPE_SMALL, gg["color"] if is_sel else COL_DIM)
		name_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		name_lbl.mouse_filter = Control.MOUSE_FILTER_IGNORE
		tab_col.add_child(name_lbl)
		tab.add_child(tab_col)
		var tsb := StyleBoxFlat.new()
		tsb.bg_color = Color(gg["color"].r * 0.22, gg["color"].g * 0.22, gg["color"].b * 0.28, 0.95) if is_sel \
				else Color(DS.BG.r, DS.BG.g, DS.BG.b, 0.88)
		tsb.border_width_bottom = 3
		tsb.border_color = gg["color"] if is_sel else Color(gg["color"].r, gg["color"].g, gg["color"].b, 0.25)
	...
	left_col.custom_minimum_size = Vector2(72, 0)
	...
	for i in range(known.size()):
		var sid: String = String(known[i])
		var def: Dictionary = KuroData.SKILL_DB[sid]
		var equipped: bool = sid in eq_set
		var full: bool = not equipped and eq_set.size() >= max_slots
		var sk := Button.new()
		sk.text = String(def["name"])
		sk.add_theme_font_size_override("font_size", 13)
		sk.custom_minimum_size = Vector2(68, 46)
		sk.disabled = full
		if equipped:
			DS.as_primary(sk)
		sk.pressed.connect(_on_skill_toggle.bind(id, sid))
		_juice(sk)
		if i % 2 == 0:
			left_col.add_child(sk)
		else:
			right_col.add_child(sk)
	...
	pr.custom_minimum_size = Vector2(0, 150)
	pr.mouse_filter = Control.MOUSE_FILTER_STOP
	pr.gui_input.connect(func(e: InputEvent):
		if (e is InputEventMouseButton and e.pressed and e.button_index == MOUSE_BUTTON_LEFT) \
				or (e is InputEventScreenTouch and e.pressed):
			_open_status(id))
	center_col.add_child(pr)

	var role_chip := PanelContainer.new()
	role_chip.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	var rcsb := StyleBoxFlat.new()
	rcsb.bg_color = Color(COL_WARM.r * 0.28, COL_WARM.g * 0.18, 0.0) if keeper else Color(0.05, 0.2, 0.1)
	rcsb.border_color = COL_WARM if keeper else DS.SUCCESS
	rcsb.set_border_width_all(1)
	rcsb.set_corner_radius_all(DS.R_SM)
	rcsb.set_content_margin_all(4)
	role_chip.add_theme_stylebox_override("panel", rcsb)
	var role_txt := _label("店 番" if keeper else "潜 行", TYPE_SMALL,
			COL_WARM if keeper else DS.SUCCESS)
	role_chip.add_child(role_txt)
	center_col.add_child(role_chip)

	hero_row.add_child(left_col)
	hero_row.add_child(center_col)
	hero_row.add_child(right_col)
	...
	var hearts: int = clampi(int(round(sim.aff(id) / 20.0)), 0, 5)
	name_col.add_child(_label("♥".repeat(hearts) + "♡".repeat(5 - hearts), TYPE_SMALL, Color("e88fb0")))
	...
	if keeper:
		...
		syn_row.add_theme_constant_override("separation", 8)
		var cur_lbl := _label("店番中", TYPE_SMALL, COL_WARM)
		syn_row.add_child(cur_lbl)
		if g.has("synergy") and String(g["synergy"]) != "":
			syn_row.add_child(_label("→ %s" % String(g["synergy"]), TYPE_SMALL, Color(1.0, 0.85, 0.5)))
		outer.add_child(syn_row)
	else:
		var kb := DS.as_primary(_button("店番にする", _on_keeper.bind(id), TYPE_BODY))
		kb.custom_minimum_size = Vector2(0, 44)
...
func _formation_banner_btn(s: Dictionary) -> PanelContainer:
	...
	var keeper_name: String = KuroData.GIRLS.get(s["morning"]["keeper"], {}).get("name", "？")
	var menu_count: int = s["morning"]["menu"].size()
	col.add_child(_label("店番: %s ／ 献立 %d品" % [keeper_name, menu_count], TYPE_SMALL, COL_DIM))
	row.add_child(col)
