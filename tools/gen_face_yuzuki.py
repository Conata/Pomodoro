#!/usr/bin/env python3
"""
ユズキの表情シートを PixAI で一括生成する。
tools/pixai_gen.py と同じ API 層を使い、expression/state ごとに個別生成して
assets/generated/face/yuzuki/ に直接配置する。
...
OUT_DIR   = Path(__file__).parent.parent / "assets/generated/face/yuzuki"
TMP_DIR   = Path(__file__).parent / "_out/face_yuzuki_raw"
REF_IMAGE = Path(__file__).parent.parent / "docs/Refs/Chara/yuzuki.png"
...
NEG = (
    ...
    "multiple girls, side view, back view, 3d"
)

# (ファイル名, プロンプト追加, シード)
VARIANTS = [
    # ── neutral ──────────────────────────────────────────────────
    ("neutral_closed", "neutral expression, mouth closed, eyes open", 1101),
    ("neutral_half",   "neutral expression, mouth slightly open, speaking, eyes open", 1102),
    ("neutral_open",   "neutral expression, mouth open wide, talking, vowel sound, eyes open", 1103),
    ("neutral_blink",  "neutral expression, mouth closed, eyes closed blinking", 1104),
    # ── smile ────────────────────────────────────────────────────
    ("smile_closed",   "big smile, happy expression, mouth closed, satisfied", 1201),
    ("smile_half",     "big smile, happy expression, mouth slightly open laughing", 1202),
    ("smile_open",     "big smile, happy expression, mouth open laughing", 1203),
    ("smile_blink",    "big smile, happy expression, eyes closed laughing", 1204),
    # ── surprise ──────────────────────────────────────────────────
    ("surprise_closed","surprised expression, wide eyes, mouth closed, shocked", 1301),
    ("surprise_half",  "surprised expression, wide eyes, mouth slightly open", 1302),
    ("surprise_open",  "surprised expression, wide eyes, mouth open wide, gasping", 1303),
    ("surprise_blink", "surprised expression, eyes shut tight, startled", 1304),
    # ── calm ──────────────────────────────────────────────────────
    ("calm_closed",    "calm focused expression, determined eyes, mouth closed", 1401),
    ("calm_half",      "calm focused expression, mouth slightly open", 1402),
    ("calm_open",      "calm focused expression, mouth open", 1403),
    ("calm_blink",     "calm expression, eyes closed, serene", 1404),
    # ── eat ───────────────────────────────────────────────────────
    ("eat_closed",     "eating food, chewing, mouth closed, cheeks puffed, happy", 1501),
    ("eat_half",       "eating food, chewing, mouth slightly open, happy", 1502),
    ("eat_open",       "eating food, mouth open taking a big bite, happy", 1503),
    ("eat_blink",      "eating food, eyes closed in delight, delicious face, yummy", 1504),
]
...
def download(url, dest: Path):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "gen_face_yuzuki/1.0"})
