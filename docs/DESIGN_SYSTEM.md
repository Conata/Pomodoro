...
## レイアウト原則：ワンスクリーン（スクロール禁止）
- 基本解像度 **720×1280・縦持ち固定**（`project.godot` で orientation=portrait・幅720固定）。
  実機は幅720固定で縦は1280以上 ＝ **横は安定、縦は必ず1280以上**。
- 各画面の内容は **`[HEADER_H, sz.y - FOOTER_H]` の1画面に収める**。スクロールは作らない
  （リストが伸びる箇所はグリッド化・段組で1画面に畳む）。
- 縦は実機で必ず余る方向なので、課題は「あふれ」ではなく「上に詰まって下が空く」こと。
  下端の要素（凡例・サマリ・育成ツリー等）は **`sz.y - FOOTER_H` 基準で下寄せ／中央寄せ**して
  1画面を使い切る（固定の 1280 前提で座標を置かない）。短い画面では `maxf`/`clampf` で
  上の節と重ならないようにフォールバックする。
  - 例（menu）：メンバーの育成ツリーはフッター直上へ下寄せ、工房のツリーマップは
    利用可能領域の中央へ、凡例は下端固定。

## 規律（self-critique）
- 型が5段を超えていないか／中央寄せ多用していないか／装飾だけの色がないか／
  CTAは1画面に1つか／余白で階層が出ているか／**スクロールに逃げていないか**（1画面に畳む）。
  出ていれば線や箱を足さない。
