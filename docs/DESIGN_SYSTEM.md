...
`T_MICRO 12` / `T_BODY 15`（基準）/ `T_SUB 18` / `T_HEAD 22` / `T_DISPLAY 40`

**自動スナップ**：`DS.txt` / `DS.tw` は渡されたサイズを `DS.snap_size()` で最寄りの5段へ
吸着させる。よって _draw ベースの全画面で「描画される文字は必ず5段のどれか」が保証される
（生の数値を渡しても可。`DS.T_*` を渡すのが意図明示として正）。計測と描画が同じ吸着を通るので
中央寄せもズレない。
